...
#include <asio/yield.hpp>
        class DeleteModKvpsOp
        {
        public:
            DeleteModKvpsOp(Modio::GameID GameID, Modio::ApiKey ApiKey, Modio::ModID ModId, std::vector<Modio::Metadata> InKvps)
                : GameID(GameID),
                  ApiKey(ApiKey),
                  ModId(ModId),
            ...
            {
                RequestParams =
                Modio::Detail::DeleteModKvpMetadataRequest.SetGameID(Modio::Detail::SDKSessionData::CurrentGameID())
                .SetModID(ModId);

                for (const Modio::Metadata& metadata : Kvps)
                {
                    RequestParams = RequestParams.AppendPayloadValue("metadata[]", fmt::format("{}:{}", metadata.Key, metadata.Value));
                }
            }
    ...
    #include <asio/unyield.hpp>
        ...
        inline void DeleteModKvps(Modio::ModID ModId, std::vector<Modio::Metadata> Kvps, Callback&& OnCallback)
        {
            return ModioAsio::async_compose<Callback, 
                                        void(Modio::ErrorCode)>(
            Modio::Detail::DeleteModKvpsOp(Modio::Detail::SDKSessionData::CurrentGameID(),
